...
class BoxTest {
    ...
    @Test
    fun `nacl box random test vectors`() {
        val nonce = ByteArray(nacl.crypto_box_NONCEBYTES)
        randomVectors.forEach { vec ->
            val pk1 = vec[0].decodeBase64()
            val sk2 = vec[1].decodeBase64()
            val msg = vec[2].decodeBase64()
            val goodBox = vec[3].decodeBase64()
            val box = nacl.box(msg, nonce, pk1, sk2)
            assertArrayEquals(goodBox, box)

            val openedBox = nacl.boxOpen(goodBox, nonce, pk1, sk2)
