...
class EthrDIDTest {
    ...
    fun `lookup owner works for new identity`() {
        ...
        val rpc = mock<JsonRPC> {
            onBlocking { ethCall(any(), any()) } doReturn "{\"result\":\"0x0000000000000000000000001122334455667788990011223344556677889900\"}"
        }
    ...
    fun `change owner sends proper transaction`() {
        ...
        val rpc = mock<JsonRPC> {
            onBlocking { getTransactionCount(any()) } doReturn BigInteger.ZERO
            onBlocking { getGasPrice() } doReturn DEFAULT_GAS_PRICE
            onBlocking { ethCall(any(), any()) } doReturn "{\"result\":\"0x0000000000000000000000001122334455667788990011223344556677889900\"}"
            onBlocking { sendRawTransaction(any()) } doReturn "mockedTxHash"
        ...
        runBlocking {
            ...
            verify(rpc).sendRawTransaction(eq(expectedSignedTx))
        }
