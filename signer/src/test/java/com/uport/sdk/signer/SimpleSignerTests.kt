...
class SimpleSignerTests {
    ...
    private val tested = SimpleSigner("65fc670d9351cb87d1f56702fb56a7832ae2aab3427be944ab8c9f2a0ab87960")

    @Test
    fun `sign ETH calls back without error`() {

        val referencePrivateKey = "3686e245890c7f997766b73a21d8e59f6385e1208831af3862574790cbc3d158"
        ...
        val signer = SimpleSigner(referencePrivateKey)
    ...
    fun `signJWT calls back with correct signature`() {
        ...
        tested.signJWT(referencePayload) { err, sigData ->
            assertNull(err)
    ...
    fun `signJWT coroutine passes`() = runBlocking {
        ...
        val sigData = tested.signJWT(referencePayload)
        Assert.assertEquals(referenceSignature, sigData)
    }

    @Test
    fun `returned address is non null`() {
        assertEquals("0x794adde0672914159c1b77dd06d047904fe96ac8", tested.getAddress().prepend0xPrefix())
    }
}